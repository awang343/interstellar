...
public:
    ...
    // The getter of the shape array of the scene;
    const std::vector<RenderShapeData>& getShapes() const;

    // The getter of light data
    const std::vector<SceneLightData>& getLights() const;

private:
    int w;
    int h;
    RenderData data; // using a shared pointer, simply copying the object
    Camera camera; // not using a shared pointer, simply copying the object
};
