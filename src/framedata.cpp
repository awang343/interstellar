...
void FrameData::loadDefaultObject() {
    ...
    o.points.push_back(std::vector<float>{0.0, 2.0, 0.0, 0.5, 0});
...
glm::vec3 FrameData::cameraAtT(float t) {
    ...
    int seg = findSegment(t, n, [&](int i) { return cameraPoints[i].w; });
...
std::vector<Sphere> FrameData::objectsAtT(float t) {
...
void FrameData::setTimeRange() {
    ...
    for (auto const &sphere : objects) {
        currentMin = fmin(currentMin, sphere.points.front()[4]);
        currentMax = fmax(currentMax, sphere.points.back()[4]) ;
    }
